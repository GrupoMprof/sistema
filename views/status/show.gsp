	...
	<body>
		...
		<div id="show-status" class="content scaffold-show" role="main">
			...
			<ol class="property-list status">
				...
				<g:if test="${statusInstance?.chamados}">
				<li class="fieldcontain">
					<span id="chamados-label" class="property-label"><g:message code="status.chamados.label" default="Chamados" /></span>
					
						<g:each in="${statusInstance.chamados}" var="c">
						<span class="property-value" aria-labelledby="chamados-label"><g:link controller="chamado" action="show" id="${c.id}">${c?.encodeAsHTML()}</g:link></span>
						</g:each>
